...
public class MaterialTextView extends AppCompatTextView {
    ...
    private void applyLineHeightFromViewAppearance(Resources.Theme theme, int resId) {
        TypedArray attributes = theme.obtainStyledAttributes(resId, C0089R.styleable.MaterialTextAppearance);
        int lineHeight = readFirstAvailableDimension(getContext(), attributes, C0089R.styleable.MaterialTextAppearance_android_lineHeight, C0089R.styleable.MaterialTextAppearance_lineHeight);
        attributes.recycle();
        if (lineHeight >= 0) {
    ...
    private static boolean canApplyTextAppearanceLineHeight(Context context) {
        return MaterialAttributes.resolveBoolean(context, C0089R.attr.textAppearanceLineHeightEnabled, true);
    }
    ...
    private static boolean viewAttrsHasLineHeight(Context context, Resources.Theme theme, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        TypedArray attributes = theme.obtainStyledAttributes(attrs, C0089R.styleable.MaterialTextView, defStyleAttr, defStyleRes);
        int lineHeight = readFirstAvailableDimension(context, attributes, C0089R.styleable.MaterialTextView_android_lineHeight, C0089R.styleable.MaterialTextView_lineHeight);
        attributes.recycle();
        if (lineHeight != -1) {
            return true;
        }
    ...
    private static int findViewAppearanceResourceId(Resources.Theme theme, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        TypedArray attributes = theme.obtainStyledAttributes(attrs, C0089R.styleable.MaterialTextView, defStyleAttr, defStyleRes);
        int appearanceAttrId = attributes.getResourceId(C0089R.styleable.MaterialTextView_android_textAppearance, -1);
        attributes.recycle();
