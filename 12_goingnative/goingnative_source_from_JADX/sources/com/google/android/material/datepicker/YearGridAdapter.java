...
class YearGridAdapter extends RecyclerView.Adapter<ViewHolder> {
    ...
    public ViewHolder onCreateViewHolder(ViewGroup viewGroup, int viewType) {
        return new ViewHolder((TextView) LayoutInflater.from(viewGroup.getContext()).inflate(C0089R.layout.mtrl_calendar_year, viewGroup, false));
    }

    public void onBindViewHolder(ViewHolder viewHolder, int position) {
        int year = getYearForPosition(position);
        String navigateYear = viewHolder.textView.getContext().getString(C0089R.string.mtrl_picker_navigate_to_year_description);
