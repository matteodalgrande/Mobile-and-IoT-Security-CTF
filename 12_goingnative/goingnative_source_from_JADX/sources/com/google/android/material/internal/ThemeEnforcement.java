...
public final class ThemeEnforcement {
    private static final int[] APPCOMPAT_CHECK_ATTRS = {C0089R.attr.colorPrimary};
    private static final String APPCOMPAT_THEME_NAME = "Theme.AppCompat";
    private static final int[] MATERIAL_CHECK_ATTRS = {C0089R.attr.colorPrimaryVariant};
    ...
    private static void checkCompatibleTheme(Context context, AttributeSet set, int defStyleAttr, int defStyleRes) {
        TypedArray a = context.obtainStyledAttributes(set, C0089R.styleable.ThemeEnforcement, defStyleAttr, defStyleRes);
        boolean enforceMaterialTheme = a.getBoolean(C0089R.styleable.ThemeEnforcement_enforceMaterialTheme, false);
        a.recycle();
        if (enforceMaterialTheme) {
            TypedValue isMaterialTheme = new TypedValue();
            if (!context.getTheme().resolveAttribute(C0089R.attr.isMaterialTheme, isMaterialTheme, true) || (isMaterialTheme.type == 18 && isMaterialTheme.data == 0)) {
    ...
    private static void checkTextAppearance(Context context, AttributeSet set, int[] attrs, int defStyleAttr, int defStyleRes, int... textAppearanceResIndices) {
        TypedArray themeEnforcementAttrs = context.obtainStyledAttributes(set, C0089R.styleable.ThemeEnforcement, defStyleAttr, defStyleRes);
        boolean validTextAppearance = false;
        if (!themeEnforcementAttrs.getBoolean(C0089R.styleable.ThemeEnforcement_enforceTextAppearance, false)) {
        ...
        } else if (themeEnforcementAttrs.getResourceId(C0089R.styleable.ThemeEnforcement_android_textAppearance, -1) != -1) {
