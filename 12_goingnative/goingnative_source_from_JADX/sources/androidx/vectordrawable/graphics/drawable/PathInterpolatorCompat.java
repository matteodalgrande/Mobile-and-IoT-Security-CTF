...
public class PathInterpolatorCompat implements Interpolator {
    ...
    /* renamed from: mX */
    private float[] f110mX;

    /* renamed from: mY */
    private float[] f111mY;
    ...
    private void initPath(Path path) {
        ...
        if (numPoints > 0) {
            this.f110mX = new float[numPoints];
            this.f111mY = new float[numPoints];
            float[] position = new float[2];
            for (int i = 0; i < numPoints; i++) {
                pathMeasure.getPosTan((((float) i) * pathLength) / ((float) (numPoints - 1)), position, (float[]) null);
                this.f110mX[i] = position[0];
                this.f111mY[i] = position[1];
            }
            if (((double) Math.abs(this.f110mX[0])) > 1.0E-5d || ((double) Math.abs(this.f111mY[0])) > 1.0E-5d || ((double) Math.abs(this.f110mX[numPoints - 1] - 1.0f)) > 1.0E-5d || ((double) Math.abs(this.f111mY[numPoints - 1] - 1.0f)) > 1.0E-5d) {
                throw new IllegalArgumentException("The Path must start at (0,0) and end at (1,1) start: " + this.f110mX[0] + "," + this.f111mY[0] + " end:" + this.f110mX[numPoints - 1] + "," + this.f111mY[numPoints - 1]);
            }
            float prevX = 0.0f;
            int componentIndex = 0;
            int i2 = 0;
            while (i2 < numPoints) {
                float[] fArr = this.f110mX;
    ...
    public float getInterpolation(float t) {
        ...
        int startIndex = 0;
        int endIndex = this.f110mX.length - 1;
        while (endIndex - startIndex > 1) {
            int midIndex = (startIndex + endIndex) / 2;
            if (t < this.f110mX[midIndex]) {
                endIndex = midIndex;
            } else {
                startIndex = midIndex;
            }
        ...
        if (xRange == 0.0f) {
            return this.f111mY[startIndex];
        }
        float[] fArr2 = this.f111mY;
