...
final class BackStackRecord extends FragmentTransaction implements FragmentManager.BackStackEntry, FragmentManagerImpl.OpGenerator {
    ...
    public void dump(String prefix, PrintWriter writer, boolean full) {
        ...
        if (!this.mOps.isEmpty()) {
            ...
            for (int opNum = 0; opNum < numOps; opNum++) {
                FragmentTransaction.C0320Op op = (FragmentTransaction.C0320Op) this.mOps.get(opNum);
                switch (op.mCmd) {
                    case 0:
                        cmdStr = "NULL";
                        break;
            ...
            if (!(container == 0 || container == lastContainer)) {
                ...
                for (int i = startIndex; i < endIndex; i++) {
                    ...
                    for (int thoseOpIndex = 0; thoseOpIndex < numThoseOps; thoseOpIndex++) {
                        FragmentTransaction.C0320Op thatOp = (FragmentTransaction.C0320Op) record.mOps.get(thoseOpIndex);
            ...
            if (i != 1) {
                if (i == 2) {
                        ...
                        if (old.mContainerId == containerId) {
                            if (old == f) {
                                ...
                                if (old == oldPrimaryNav) {
                                    this.mOps.add(opNum, new FragmentTransaction.C0320Op(9, old));
                                    opNum++;
                                    oldPrimaryNav = null;
                                }
                                FragmentTransaction.C0320Op removeOp = new FragmentTransaction.C0320Op(3, old);
                    ...
                    if (op.mFragment == oldPrimaryNav) {
                        this.mOps.add(opNum, new FragmentTransaction.C0320Op(9, op.mFragment));
                        opNum++;
                        oldPrimaryNav = null;
                    }
                } else if (i != 7) {
                    if (i == 8) {
                        this.mOps.add(opNum, new FragmentTransaction.C0320Op(9, oldPrimaryNav));
                        opNum++;
    ...
    public boolean isPostponed() {
        for (int opNum = 0; opNum < this.mOps.size(); opNum++) {
            if (isFragmentPostponed((FragmentTransaction.C0320Op) this.mOps.get(opNum))) {
                return true;
            }
        }
        return false;
    ...
    private static boolean isFragmentPostponed(FragmentTransaction.C0320Op op) {
