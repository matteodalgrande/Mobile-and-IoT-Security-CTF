...
public class FontResourcesParserCompat {
    ...
    private static FamilyResourceEntry readFamily(XmlPullParser parser, Resources resources) throws XmlPullParserException, IOException {
        TypedArray array = resources.obtainAttributes(Xml.asAttributeSet(parser), C0026R.styleable.FontFamily);
        String authority = array.getString(C0026R.styleable.FontFamily_fontProviderAuthority);
        String providerPackage = array.getString(C0026R.styleable.FontFamily_fontProviderPackage);
        String query = array.getString(C0026R.styleable.FontFamily_fontProviderQuery);
        int certsId = array.getResourceId(C0026R.styleable.FontFamily_fontProviderCerts, 0);
        int strategy = array.getInteger(C0026R.styleable.FontFamily_fontProviderFetchStrategy, 1);
        int timeoutMs = array.getInteger(C0026R.styleable.FontFamily_fontProviderFetchTimeout, DEFAULT_TIMEOUT_MILLIS);
        array.recycle();
    ...
    private static FontFileResourceEntry readFont(XmlPullParser parser, Resources resources) throws XmlPullParserException, IOException {
        ...
        int i2;
        TypedArray array = resources.obtainAttributes(Xml.asAttributeSet(parser), C0026R.styleable.FontFamilyFont);
        if (array.hasValue(C0026R.styleable.FontFamilyFont_fontWeight)) {
            weightAttr = C0026R.styleable.FontFamilyFont_fontWeight;
        } else {
            weightAttr = C0026R.styleable.FontFamilyFont_android_fontWeight;
        }
        int weight = array.getInt(weightAttr, NORMAL_WEIGHT);
        if (array.hasValue(C0026R.styleable.FontFamilyFont_fontStyle)) {
            styleAttr = C0026R.styleable.FontFamilyFont_fontStyle;
        } else {
            styleAttr = C0026R.styleable.FontFamilyFont_android_fontStyle;
        }
        boolean isItalic = 1 == array.getInt(styleAttr, 0);
        if (array.hasValue(C0026R.styleable.FontFamilyFont_ttcIndex)) {
            i = C0026R.styleable.FontFamilyFont_ttcIndex;
        } else {
            i = C0026R.styleable.FontFamilyFont_android_ttcIndex;
        }
        int ttcIndexAttr = i;
        if (array.hasValue(C0026R.styleable.FontFamilyFont_fontVariationSettings)) {
            variationSettingsAttr = C0026R.styleable.FontFamilyFont_fontVariationSettings;
        } else {
            variationSettingsAttr = C0026R.styleable.FontFamilyFont_android_fontVariationSettings;
        }
        String variationSettings = array.getString(variationSettingsAttr);
        int ttcIndex = array.getInt(ttcIndexAttr, 0);
        if (array.hasValue(C0026R.styleable.FontFamilyFont_font)) {
            i2 = C0026R.styleable.FontFamilyFont_font;
        } else {
            i2 = C0026R.styleable.FontFamilyFont_android_font;
        }
        int resourceAttr = i2;
