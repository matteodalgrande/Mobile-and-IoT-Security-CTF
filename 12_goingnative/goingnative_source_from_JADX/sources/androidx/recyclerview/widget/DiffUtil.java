...
public class DiffUtil {
    private static final Comparator<Snake> SNAKE_COMPARATOR = new Comparator<Snake>() {
        public int compare(Snake o1, Snake o2) {
            int cmpX = o1.f101x - o2.f101x;
            return cmpX == 0 ? o1.f102y - o2.f102y : cmpX;
        }
    };
    ...
    public static DiffResult calculateDiff(Callback cb, boolean detectMoves) {
        ...
        while (!stack.isEmpty()) {
            ...
            if (snake != null) {
                ...
                snake.f101x += range.oldListStart;
                snake.f102y += range.newListStart;
                Range left = arrayList.isEmpty() ? new Range() : (Range) arrayList.remove(arrayList.size() - 1);
                left.oldListStart = range.oldListStart;
                left.newListStart = range.newListStart;
                if (snake.reverse) {
                    left.oldListEnd = snake.f101x;
                    left.newListEnd = snake.f102y;
                } else if (snake.removal) {
                    left.oldListEnd = snake.f101x - 1;
                    left.newListEnd = snake.f102y;
                } else {
                    left.oldListEnd = snake.f101x;
                    left.newListEnd = snake.f102y - 1;
                }
                stack.add(left);
                Range right = range;
                if (!snake.reverse) {
                    right.oldListStart = snake.f101x + snake.size;
                    right.newListStart = snake.f102y + snake.size;
                } else if (snake.removal) {
                    right.oldListStart = snake.f101x + snake.size + 1;
                    right.newListStart = snake.f102y + snake.size;
                } else {
                    right.oldListStart = snake.f101x + snake.size;
                    right.newListStart = snake.f102y + snake.size + 1;
                }
                stack.add(right);
            } else {
                arrayList.add(range);
            ...
            for (int d = 0; d <= dLimit; d++) {
                ...
                while (k <= d) {
                    ...
                    if (!checkInFwd || k < (delta - d) + 1 || k > (delta + d) - 1 || iArr[kOffset + k] < iArr2[kOffset + k]) {
                        ...
                        outSnake.f101x = iArr2[kOffset + k];
                        outSnake.f102y = outSnake.f101x - k;
                ...
                while (k2 <= d) {
                    ...
                    if (checkInFwd || k2 + delta < (-d) || k2 + delta > d || iArr[kOffset + backwardK] < iArr2[kOffset + backwardK]) {
                        ...
                        outSnake2.f101x = iArr2[kOffset + backwardK];
                        outSnake2.f102y = outSnake2.f101x - backwardK;
    ...
    public static class DiffResult {
        ...
        private void addRootSnake() {
            ...
            if (firstSnake == null || firstSnake.f101x != 0 || firstSnake.f102y != 0) {
        ...
        private void findMatchingItems() {
            ...
            for (int i = this.mSnakes.size() - 1; i >= 0; i--) {
                ...
                int endX = snake.f101x + snake.size;
                int endY = snake.f102y + snake.size;
                ...
                for (int j = 0; j < snake.size; j++) {
                    int oldItemPos = snake.f101x + j;
                    int newItemPos = snake.f102y + j;
        ...
        public void dispatchUpdatesTo(ListUpdateCallback updateCallback) {
            ...
            for (int snakeIndex = this.mSnakes.size() - 1; snakeIndex >= 0; snakeIndex--) {
                ...
                int endX = snake.f101x + snakeSize2;
                int endY2 = snake.f102y + snakeSize2;
                if (endX < posOld) {
                    endY = endY2;
                ...
                for (int i2 = snakeSize - 1; i2 >= 0; i2--) {
                    if ((this.mOldItemStatuses[snake.f101x + i2] & 31) == 2) {
                        batchingCallback.onChanged(snake.f101x + i2, 1, this.mCallback.getChangePayload(snake.f101x + i2, snake.f102y + i2));
                    }
                }
                posOld = snake.f101x;
                posNew = snake.f102y;
