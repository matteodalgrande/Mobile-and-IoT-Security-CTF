...
public class Easing {
    ...
    static class CubicEasing extends Easing {
        ...
        CubicEasing(String configString) {
            ...
            this.f55x1 = Double.parseDouble(configString.substring(start + 1, off1).trim());
            int off2 = configString.indexOf(44, off1 + 1);
            this.f57y1 = Double.parseDouble(configString.substring(off1 + 1, off2).trim());
            int off3 = configString.indexOf(44, off2 + 1);
            this.f56x2 = Double.parseDouble(configString.substring(off2 + 1, off3).trim());
            this.f58y2 = Double.parseDouble(configString.substring(off3 + 1, configString.indexOf(41, off3 + 1)).trim());
        }
        ...
        private double getX(double t) {
            double t1 = 1.0d - t;
            return (this.f55x1 * t1 * 3.0d * t1 * t) + (this.f56x2 * 3.0d * t1 * t * t) + (t * t * t);
        }

        private double getY(double t) {
            double t1 = 1.0d - t;
            return (this.f57y1 * t1 * 3.0d * t1 * t) + (this.f58y2 * 3.0d * t1 * t * t) + (t * t * t);
        }

        private double getDiffX(double t) {
            double t1 = 1.0d - t;
            double d = this.f55x1;
            double d2 = this.f56x2;
