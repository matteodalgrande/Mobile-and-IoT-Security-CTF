...
public class LinearCurveFit extends CurveFit {
    ...
    /* renamed from: mT */
    private double[] f63mT;
    private double mTotalLength = Double.NaN;

    /* renamed from: mY */
    private double[][] f64mY;
    ...
    private double getLength2D(double t) {
        ...
        for (int i = 0; i < n - 1; i++) {
            ...
            last_x = px;
            last_y = py;
            double[] dArr3 = this.f63mT;
            if (t == dArr3[i]) {
                return sum;
            }
            if (t < dArr3[i + 1]) {
                double x = (t - dArr3[i]) / (dArr3[i + 1] - dArr3[i]);
                double[][] dArr4 = this.f64mY;
                double x1 = dArr4[i][0];
    ...
    public void getPos(double t, double[] v) {
        ...
        if (t <= dArr[0]) {
            ...
            for (int j2 = 0; j2 < dim; j2++) {
                v[j2] = this.f64mY[n - 1][j2];
            }
        } else {
            for (int i = 0; i < n - 1; i++) {
                if (t == this.f63mT[i]) {
                    for (int j3 = 0; j3 < dim; j3++) {
                        v[j3] = this.f64mY[i][j3];
                    }
                }
                double[] dArr2 = this.f63mT;
                if (t < dArr2[i + 1]) {
                    double x = (t - dArr2[i]) / (dArr2[i + 1] - dArr2[i]);
                    for (int j4 = 0; j4 < dim; j4++) {
                        double[][] dArr3 = this.f64mY;
    ...
    public void getPos(double t, float[] v) {
        ...
        if (t <= dArr[0]) {
            for (int j = 0; j < dim; j++) {
                v[j] = (float) this.f64mY[0][j];
            }
        } else if (t >= dArr[n - 1]) {
            for (int j2 = 0; j2 < dim; j2++) {
                v[j2] = (float) this.f64mY[n - 1][j2];
            }
        } else {
            for (int i = 0; i < n - 1; i++) {
                if (t == this.f63mT[i]) {
                    for (int j3 = 0; j3 < dim; j3++) {
                        v[j3] = (float) this.f64mY[i][j3];
                    }
                }
                double[] dArr2 = this.f63mT;
                if (t < dArr2[i + 1]) {
    ...
    public double getPos(double t, int j) {
        ...
        int n = dArr.length;
        if (t <= dArr[0]) {
            return this.f64mY[0][j];
        }
        if (t >= dArr[n - 1]) {
            return this.f64mY[n - 1][j];
        }
        for (int i = 0; i < n - 1; i++) {
            double[] dArr2 = this.f63mT;
            if (t == dArr2[i]) {
                return this.f64mY[i][j];
            }
            if (t < dArr2[i + 1]) {
                double x = (t - dArr2[i]) / (dArr2[i + 1] - dArr2[i]);
                double[][] dArr3 = this.f64mY;
