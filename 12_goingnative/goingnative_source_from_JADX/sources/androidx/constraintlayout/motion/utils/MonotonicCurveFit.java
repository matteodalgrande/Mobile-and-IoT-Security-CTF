...
public class MonotonicCurveFit extends CurveFit {
    ...
    /* renamed from: mT */
    private double[] f65mT;
    private double[][] mTangent;

    /* renamed from: mY */
    private double[][] f66mY;
    ...
    public void getPos(double t, double[] v) {
        double[] dArr = this.f65mT;
        int n = dArr.length;
        int dim = this.f66mY[0].length;
        if (t <= dArr[0]) {
            for (int j = 0; j < dim; j++) {
                v[j] = this.f66mY[0][j];
            }
    ...
    public void getPos(double t, float[] v) {
        ...
        if (t <= dArr[0]) {
            for (int j = 0; j < dim; j++) {
                v[j] = (float) this.f66mY[0][j];
            }
        } else if (t >= dArr[n - 1]) {
            for (int j2 = 0; j2 < dim; j2++) {
                v[j2] = (float) this.f66mY[n - 1][j2];
            }
        } else {
            for (int i = 0; i < n - 1; i++) {
                if (t == this.f65mT[i]) {
                    for (int j3 = 0; j3 < dim; j3++) {
                        v[j3] = (float) this.f66mY[i][j3];
                    }
                }
                double[] dArr2 = this.f65mT;
                if (t < dArr2[i + 1]) {
    ...
    public double getPos(double t, int j) {
        ...
        int n = dArr.length;
        if (t <= dArr[0]) {
            return this.f66mY[0][j];
        }
        if (t >= dArr[n - 1]) {
            return this.f66mY[n - 1][j];
        }
        for (int i = 0; i < n - 1; i++) {
            double[] dArr2 = this.f65mT;
            if (t == dArr2[i]) {
                return this.f66mY[i][j];
            }
            if (t < dArr2[i + 1]) {
                double h = dArr2[i + 1] - dArr2[i];
                double[][] dArr3 = this.f66mY;
