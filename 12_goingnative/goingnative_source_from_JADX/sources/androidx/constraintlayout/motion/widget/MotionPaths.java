...
class MotionPaths implements Comparable<MotionPaths> {
    ...
    public void initCartesian(KeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        float position4 = motionPaths.f72y;
        float endCenterX = motionPaths2.f71x + (f / 2.0f);
        float endCenterY = motionPaths2.f72y + (f3 / 2.0f);
        float pathVectorX = endCenterX - (f5 + (f2 / 2.0f));
        float pathVectorY = endCenterY - (position4 + (f4 / 2.0f));
        this.f71x = (float) ((int) ((f5 + (pathVectorX * path)) - ((scaleX * scaleWidth) / 2.0f)));
        this.f72y = (float) ((int) ((position4 + (pathVectorY * path)) - ((scaleY * scaleHeight) / 2.0f)));
        ...
        this.mMode = 2;
        this.f71x = (float) ((int) (((motionPaths.f71x + (pathVectorX * dxdx)) + (pathVectorY * dxdy)) - ((scaleX * scaleWidth) / 2.0f)));
        this.f72y = (float) ((int) (((motionPaths.f72y + (pathVectorX * dydx)) + (pathVectorY * dydy)) - ((scaleY * scaleHeight) / 2.0f)));
    ...
    public void initScreen(int parentWidth, int parentHeight, KeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        float f5 = motionPaths.f71x;
        float f6 = position2;
        float position3 = motionPaths.f72y;
        float endCenterX = motionPaths2.f71x + (f / 2.0f);
        float endCenterY = motionPaths2.f72y + (f3 / 2.0f);
        this.f71x = (float) ((int) ((f5 + ((endCenterX - (f5 + (f2 / 2.0f))) * path)) - ((scaleX * scaleWidth) / 2.0f)));
        this.f72y = (float) ((int) ((position3 + ((endCenterY - (position3 + (f4 / 2.0f))) * path)) - ((scaleY * scaleHeight) / 2.0f)));
        this.width = (float) ((int) (f2 + (scaleX * scaleWidth)));
        this.height = (float) ((int) (f4 + (scaleY * scaleHeight)));
        this.mMode = 3;
        if (!Float.isNaN(keyPosition.mPercentX)) {
            this.f71x = (float) ((int) (keyPosition.mPercentX * ((float) ((int) (((float) parentWidth) - this.width)))));
        }
        if (!Float.isNaN(keyPosition.mPercentY)) {
            this.f72y = (float) ((int) (keyPosition.mPercentY * ((float) ((int) (((float) parentHeight) - this.height)))));
        }
        this.mKeyFrameEasing = Easing.getInterpolator(keyPosition.mTransitionEasing);
        this.mPathMotionArc = keyPosition.mPathMotionArc;
    }
    ...
    public void initPath(KeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        float f = motionPaths.f71x;
        float f2 = motionPaths.width;
        float f3 = motionPaths.f72y;
        float f4 = position2;
        float position3 = motionPaths.height;
        float endCenterX = motionPaths2.f71x + (motionPaths2.width / 2.0f);
        float endCenterY = motionPaths2.f72y + (motionPaths2.height / 2.0f);
        float pathVectorX = endCenterX - ((f2 / 2.0f) + f);
        float pathVectorY = endCenterY - (f3 + (position3 / 2.0f));
        this.f71x = (float) ((int) ((f + (pathVectorX * path)) - ((scaleX * scaleWidth) / 2.0f)));
        this.f72y = (float) ((int) ((f3 + (pathVectorY * path)) - ((scaleY * scaleHeight) / 2.0f)));
        this.width = (float) ((int) (f2 + (scaleX * scaleWidth)));
        ...
        float f6 = (float) ((int) ((motionPaths3.f71x + (pathVectorX * path)) - ((scaleX * scaleWidth) / 2.0f)));
        this.f71x = f6;
        float f7 = pathVectorX;
        float f8 = (float) ((int) ((motionPaths3.f72y + (pathVectorY * path)) - ((scaleY * scaleHeight) / 2.0f)));
        this.f72y = f8;
        this.f71x = f6 + ((-pathVectorY) * perpendicular);
        this.f72y = f8 + (pathVectorX * perpendicular);
    ...
    public void different(MotionPaths points, boolean[] mask, String[] custom, boolean arcMode) {
        ...
        int c2 = c + 1;
        mask[c] = mask[c] | diff(this.f71x, points.f71x) | arcMode;
        int c3 = c2 + 1;
        mask[c2] = mask[c2] | diff(this.f72y, points.f72y) | arcMode;
        int c4 = c3 + 1;
    ...
    public void fillStandard(double[] data, int[] toUse) {
        float[] set = {this.position, this.f71x, this.f72y, this.width, this.height, this.mPathRotate};
        int c = 0;
        for (int i = 0; i < toUse.length; i++) {
