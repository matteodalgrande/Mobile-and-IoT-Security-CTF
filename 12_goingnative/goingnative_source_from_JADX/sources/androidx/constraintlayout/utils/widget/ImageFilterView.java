...
public class ImageFilterView extends AppCompatImageView {
    ...
    static class ImageMatrix {
        ...
        public void updateMatrix(ImageView view) {
            ...
            if (f != 1.0f) {
                saturation(f);
                this.mColorMatrix.set(this.f88m);
                filter = true;
            }
            ...
            if (f3 != 1.0f) {
                warmth(f3);
                this.mTmpColorMatrix.set(this.f88m);
                this.mColorMatrix.postConcat(this.mTmpColorMatrix);
                filter = true;
            }
    ...
    private void init(Context context, AttributeSet attrs) {
        if (attrs != null) {
            TypedArray a = getContext().obtainStyledAttributes(attrs, C0017R.styleable.ImageFilterView);
            int N = a.getIndexCount();
            Drawable drawable = a.getDrawable(C0017R.styleable.ImageFilterView_altSrc);
            for (int i = 0; i < N; i++) {
                int attr = a.getIndex(i);
                if (attr == C0017R.styleable.ImageFilterView_crossfade) {
                    this.mCrossfade = a.getFloat(attr, 0.0f);
                } else if (attr == C0017R.styleable.ImageFilterView_warmth) {
                    setWarmth(a.getFloat(attr, 0.0f));
                } else if (attr == C0017R.styleable.ImageFilterView_saturation) {
                    setSaturation(a.getFloat(attr, 0.0f));
                } else if (attr == C0017R.styleable.ImageFilterView_contrast) {
                    setContrast(a.getFloat(attr, 0.0f));
                } else if (attr == C0017R.styleable.ImageFilterView_round) {
                    if (Build.VERSION.SDK_INT >= 21) {
                        setRound(a.getDimension(attr, 0.0f));
                    }
                } else if (attr == C0017R.styleable.ImageFilterView_roundPercent) {
                    if (Build.VERSION.SDK_INT >= 21) {
                        setRoundPercent(a.getFloat(attr, 0.0f));
                    }
                } else if (attr == C0017R.styleable.ImageFilterView_overlay) {
